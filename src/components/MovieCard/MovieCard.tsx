...
const MovieCard: React.FC<IMovieCard> = ({
  ...
  return (
    <div
      className="bg-black bg-opacity-15 px-0 mx-4 my-2 w-64 auto float-left overflow-hidden block relative shadow-lg rounded-lg flex-shrink-0 smooth-scroll flex-none"
      onClick={() => {
        navigateMovies(movieId, title);
      }}
    >
      <div className="bg-white auto float-left overflow-hidden block relative shadow-lg rounded-lg flex-shrink-0 smooth-scroll">
        <div className=" min-w-full overflow-hidden bg-gray-800 float-none transition-opacity duration-5550 ease-in-out transform-gpu">
          <img
            src={poster}
            className="max-h-96 transition-all duration-900 ease-in-out backface-hidden overflow-hidden min-w-full max-h-poster relative max-w-none  scale-100 hover:scale-125 hover:opacity-40"
          />
        </div>
